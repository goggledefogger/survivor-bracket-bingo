  ...
  return (
    <div className="space-y-6">
      <header className="text-center">
        <h2 className="font-display text-4xl tracking-wider text-sand-warm">Tovo Dashboard</h2>
        <p className="text-sand-warm/60 text-sm mt-1">Live Tribal Council & Challenge Statistics</p>
      </header>

      <section>
        <FijianSectionHeader title="Top Survivors" />
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {ranked.map(({ name, idx, score }, rank) => (
            <article
              key={idx}
              className="bg-stone-900/80 border border-ochre/20 rounded-xl p-4 hover:shadow-fire transition-all"
              style={{ borderTopColor: PLAYER_COLORS[idx].hex, borderTopWidth: '3px' }}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-sand-warm/50">#{rank + 1}</span>
                {rank === 0 && score > 0 && <Icon name="workspace_premium" className="text-ochre" aria-hidden />}
              </div>
              ...
              <div className="text-sand-warm/50 text-xs mt-1">points</div>
              <div className="mt-3 flex flex-wrap gap-1">
                {(playerPicks[idx] || []).map((cId) => {
                  ...
                  return (
                    <span
                      key={cId}
                      className={`text-xs px-2 py-0.5 rounded bg-stone-800/50 ${isEliminated ? 'line-through opacity-40' : 'text-sand-warm/70'}`}
                    >
                      {c?.name?.split(' ')[0] || cId}
                    </span>
                  );
      ...
      <section>
        <FijianSectionHeader title="Score Events" />
        <FijianCard className="p-6">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <select
              value={selectedCastaway}
              onChange={(e) => setSelectedCastaway(e.target.value)}
              className="flex-1 min-w-[140px] px-3 py-2 rounded-lg border border-ochre/30 bg-stone-dark text-sand-warm text-sm outline-none focus:border-ochre"
            ...
            <button
              type="button"
              onClick={undoLast}
              className="px-3 py-2 text-xs rounded-lg border border-ochre/30 bg-stone-dark text-sand-warm/70 hover:text-ochre hover:border-ochre/50 transition-all"
            >
              Undo Last
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {SCORE_EVENTS.map((ev) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs transition-all flex items-center gap-1.5 disabled:opacity-30 disabled:cursor-default
                  ${ev.negative
                      ? 'border border-fire-600/30 bg-stone-800 text-sand-warm/70 hover:border-fire-600 hover:text-fire-400'
                      : 'border border-ochre/30 bg-stone-800 text-sand-warm/70 hover:border-ochre/50 hover:text-ochre'}
                `}
              >
                <Icon name={EVENT_ICONS[ev.key] || 'circle'} className="text-sm" />
                {ev.label} ({ev.points > 0 ? '+' : ''}{ev.points})
              </button>
            ))}
          </div>
          ...
          {eventLog.length > 0 && (
            <div className="mt-4 space-y-1">
              <p className="text-sand-warm/50 text-xs mb-1">Recent:</p>
              {eventLog.slice(0, 5).map((e, i) => {
                const c = ALL_CASTAWAYS.find((x) => x.id === e.castawayId);
                return (
                  <div key={i} className="text-xs text-sand-warm/60 flex items-center gap-2">
                    <span className={e.points > 0 ? 'text-jungle-400' : 'text-fire-400'}>
                      {e.points > 0 ? '+' : ''}{e.points}
                    </span>
                    <span className="text-sand-warm/80">{c?.name}</span>
                    <span>— {e.label}</span>
                  </div>
                );
              })}
